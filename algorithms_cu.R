...
crs_utm <- 25829  # ETRS89 / UTM 29N (metros)

resolve_target_col <- function(nms) {
  cand <- c("Cu_ppm", "Cu__ppm_", "Cu (ppm)")
  hit <- cand[cand %in% nms]
  if (length(hit) == 0) stop("No encuentro columna de Cu (Cu_ppm / Cu__ppm_ / 'Cu (ppm)')")
  hit[1]
}
...
# Para resolver K^{-1} b con cholesky (R)
...
# Cov Matérn 3/2 (sigma^2 * (1 + a) exp(-a))
...
# Boundary a segment para INLA
huelva.bdry <- andalucia_huelva |>
  as("Spatial") |>
  inla.sp2segment()

# Mesh (ajusta si quieres)
...
# Fórmula explícita
covar_terms <- colnames(X_obs_df)  # Intercept + d_*
...
# Predicciones en grilla (en escala log)
...
# Transformación a escala Cu (ppm) usando expm1
cu_pred_inla_mean <- expm1(mu_pred_inla_mean)
cu_pred_inla_q025 <- expm1(mu_pred_inla_q025)
cu_pred_inla_q975 <- expm1(mu_pred_inla_q975)

grid_huelva$mu_inla_mean <- mu_pred_inla_mean
grid_huelva$mu_inla_sd   <- mu_pred_inla_sd
grid_huelva$cu_inla_mean <- cu_pred_inla_mean
grid_huelva$cu_inla_q025 <- cu_pred_inla_q025
grid_huelva$cu_inla_q975 <- cu_pred_inla_q975

# Resumen posterior parámetros INLA
cat("\nINLA fixed effects:\n")
...
grid_huelva$cu_mcmc_mean <- expm1(mu_pred_mcmc_mean)
...
summ_vec <- function(v) c(mean=mean(v), sd=sd(v), q025=quantile(v,0.025), q975=quantile(v,0.975))
print(rbind(
...
grid_huelva$cu_vb_mean <- expm1(mu_pred_vb_mean)
...
# Asegurar columnas X,Y en grid_huelva (si el sf no las trae como columnas)
# (Si X,Y ya existen, no hace nada)
...
p_inla <- make_map(grid_huelva, "cu_inla_mean", "Cu (ppm) – Predicción", "INLA/SPDE")
p_mcmc <- make_map(grid_huelva, "cu_mcmc_mean", "Cu (ppm) – Predicción", "MCMC (spBayes)")
p_vb   <- make_map(grid_huelva, "cu_vb_mean",   "Cu (ppm) – Predicción", "VB (Stan)")
...
df_vb_hyp <- bind_rows(
  data.frame(t(summ_vec(post_vb_h$sigma)), metodo="VB", tipo="spatial", param="sigma", stringsAsFactors = FALSE),
  data.frame(t(summ_vec(post_vb_h$rho)),   metodo="VB", tipo="spatial", param="rho",   stringsAsFactors = FALSE),
  data.frame(t(summ_vec(post_vb_h$tau)),   metodo="VB", tipo="spatial", param="tau",   stringsAsFactors = FALSE)
) %>%
...
p_mean_log <- plot_grid(
  make_map_points_log(grid_huelva, "mu_inla_mean",
                      "Cu – Posterior mean (log scale)", "INLA / SPDE", "Mean Cu"),
  make_map_points_log(grid_huelva, "mu_mcmc_mean",
                      "Cu – Posterior mean (log scale)", "MCMC (spBayes)", "Mean Cu"),
  make_map_points_log(grid_huelva, "mu_vb_mean",
                      "Cu – Posterior mean (log scale)", "VB (Stan)", "Mean Cu"),
  ncol = 3
)
...
ggsave(
  filename = "cu_posterior_mean_logscale.png",
  plot     = p_mean_log,
  width    = 14,
  height   = 5,
  dpi      = 300
...
p_sd_log <- plot_grid(
  make_map_points_log(grid_huelva, "mu_inla_sd",
                      "Cu – Posterior SD (log scale)", "INLA / SPDE", "SD Cu"),
  make_map_points_log(grid_huelva, "mu_mcmc_sd",
                      "Cu – Posterior SD (log scale)", "MCMC (spBayes)", "SD Cu"),
  make_map_points_log(grid_huelva, "mu_vb_sd",
                      "Cu – Posterior SD (log scale)", "VB (Stan)", "SD Cu"),
  ncol = 3
)
...
ggsave(
  filename = "cu_posterior_sd_logscale.png",
  plot     = p_sd_log,
  width    = 14,
  height   = 5,
  dpi      = 300
